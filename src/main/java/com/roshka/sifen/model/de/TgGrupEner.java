...
public class TgGrupEner {
    ...
    public void setupSOAPElements(SOAPElement gCamEsp) throws SOAPException {
        SOAPElement gGrupEner = gCamEsp.addChildElement("gGrupEner", Constants.SIFEN_NS_PREFIX);
        if (this.dNroMed != null)
            gGrupEner.addChildElement("dNroMed", Constants.SIFEN_NS_PREFIX).setTextContent(this.dNroMed);

        if (this.dActiv != 0)
            gGrupEner.addChildElement("dActiv", Constants.SIFEN_NS_PREFIX).setTextContent(String.valueOf(this.dActiv));

        if (this.dCateg != null)
            gGrupEner.addChildElement("dCateg", Constants.SIFEN_NS_PREFIX).setTextContent(this.dCateg);

        if (this.dLecAnt != null)
            gGrupEner.addChildElement("dLecAnt", Constants.SIFEN_NS_PREFIX).setTextContent(String.valueOf(this.dLecAnt));

        if (this.dLecAct != null)
            gGrupEner.addChildElement("dLecAct", Constants.SIFEN_NS_PREFIX).setTextContent(String.valueOf(this.dLecAct));

        if (this.dConKwh != null)
            gGrupEner.addChildElement("dConKwh", Constants.SIFEN_NS_PREFIX).setTextContent(String.valueOf(this.dConKwh));
    }
